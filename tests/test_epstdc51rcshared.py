import unittest
import numpy as np
import jax
import jax.numpy as jnp

from slimdqn.algorithms.epstdc51rcshared import EPSTDC51RCShared
from tests.utils import Generator


def categorical_projection(next_probs, reward, is_terminal, gamma_h, vmin, vmax, n_bins):
    support = np.linspace(vmin, vmax, n_bins)
    dz = (vmax - vmin) / (n_bins - 1)
    tz = np.clip(reward + (1.0 - is_terminal) * gamma_h * support, vmin, vmax)
    b = (tz - vmin) / dz
    lower = np.floor(b).astype(np.int32)
    upper = np.ceil(b).astype(np.int32)
    m = np.zeros(n_bins)
    for j in range(n_bins):
        m[lower[j]] += next_probs[j] * (upper[j] - b[j])
        m[upper[j]] += next_probs[j] * (b[j] - lower[j])
        m[lower[j]] += next_probs[j] * (lower[j] == upper[j])
    return m


class TestEPSTDC51RCShared(unittest.TestCase):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.random_seed = np.random.randint(1000)
        self.key = jax.random.PRNGKey(self.random_seed)

        key_actions, key_feature_1, key_feature_2, key_feature_3, key_feature_4 = jax.random.split(self.key, 5)
        self.observation_dim = (84, 84, 4)
        self.n_actions = int(jax.random.randint(key_actions, (), minval=2, maxval=10))
        self.q = EPSTDC51RCShared(
            self.key,
            self.observation_dim,
            self.n_actions,
            [
                jax.random.randint(key_feature_1, (), minval=1, maxval=10),
                jax.random.randint(key_feature_2, (), minval=1, maxval=10),
                jax.random.randint(key_feature_3, (), minval=1, maxval=10),
                jax.random.randint(key_feature_4, (), minval=1, maxval=10),
            ],
            0.001,
            0.94,
            1,
            1,
            1,
            1,
        )
        self.n_bins = self.q.n_bins
        self.vmin, self.vmax = -10, 10
        self.support = jnp.linspace(self.vmin, self.vmax, self.n_bins)

        self.generator = Generator(None, self.observation_dim, self.n_actions)

    def test_compute_target(self) -> None:
        print(f"-------------- Random key {self.random_seed} --------------")
        sample = self.generator.sample(self.key)

        computed_target = self.q.compute_target(self.q.params, sample)  # (n_bins,)

        q_next_probs = jax.nn.softmax(
            self.q.network.apply(self.q.params, sample.next_state)[0].reshape(self.n_actions, self.n_bins), axis=-1
        )
        next_q_values = np.array(q_next_probs @ self.q.support)  # (n_actions,)
        best_action_index = int(np.argmax(next_q_values))
        target = categorical_projection(
            np.array(q_next_probs[best_action_index]),
            float(sample.reward),
            float(sample.is_terminal),
            self.q.gamma**self.q.update_horizon,
            self.vmin,
            self.vmax,
            self.n_bins,
        )

        np.testing.assert_array_equal(self.q.support, self.support)
        self.assertEqual(computed_target.shape, (self.n_bins,))
        np.testing.assert_allclose(np.array(computed_target), target, atol=1e-4)
        np.testing.assert_allclose(float(np.array(computed_target).sum()), 1.0, atol=1e-4)

    def test_loss(self) -> None:
        print(f"-------------- Random key {self.random_seed} --------------")
        sample = self.generator.sample(self.key)

        computed_loss = self.q.loss(self.q.params, sample, jnp.ones(1))[0]

        q_logits, h_logits = self.q.network.apply(self.q.params, sample.state)
        q_logits = q_logits.reshape(self.n_actions, self.n_bins)[sample.action]  # (n_bins,)
        h_logits = h_logits.reshape(self.n_actions, self.n_bins)[sample.action]  # (n_bins,)
        q_log_distribution = jax.nn.log_softmax(q_logits, axis=-1)

        target = self.q.compute_target(self.q.params, sample)  # (n_bins,)

        cross_entropy = -jnp.sum(target * q_log_distribution)
        td_loss = jnp.sum(target * h_logits) + cross_entropy
        h_loss = jnp.sum(target * h_logits) - jax.scipy.special.logsumexp(h_logits + q_log_distribution)
        expected = td_loss - h_loss

        np.testing.assert_allclose(float(expected), float(computed_loss.sum()), rtol=1e-4, atol=1e-4)

    def test_best_action(self):
        print(f"-------------- Random key {self.random_seed} --------------")
        state = self.generator.state(self.key)

        computed_q_action, computed_h_action = self.q.best_action(self.q.params, state)

        q_logits, h_logits = self.q.network.apply(self.q.params, state)
        q_logits = q_logits.reshape(self.n_actions, self.n_bins)
        h_logits = h_logits.reshape(self.n_actions, self.n_bins)

        q_values = jax.nn.softmax(q_logits, axis=-1) @ self.q.support  # (n_actions,)
        h_values = jax.nn.softmax(h_logits, axis=-1) @ self.q.support  # (n_actions,)
        q_action = jnp.argmax(q_values)
        h_action = jnp.argmax(jnp.abs(h_values)) # eps-TD: argmax |expected H|

        self.assertEqual(q_values.shape, (self.n_actions,))
        self.assertEqual(q_action, computed_q_action)
        self.assertEqual(h_action, computed_h_action)