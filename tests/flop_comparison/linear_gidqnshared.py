...
from tests.flop_comparison.architectures.lineardqn import LinearDQNNet
...
def set_target_params(params, n_actions):
    return optax.tree_utils.tree_set(
        params, q_heads=jax.tree.map(lambda p: p[..., :n_actions], params["params"]["q_heads"])
    )
...
class LinearGiDQNShared:
    def __init__(
        ...
        self.online_networks = LinearDQNNet(
            features,
            architecture_type,
            layer_norm,
            gap,
        ...
        self.root_network = LinearDQNNet(
