...
class LinearDQNNet(nn.Module):
    ...
    def __call__(self, x):
        ...
        q_vals = nn.Dense(self.n_actions * self.n_heads, kernel_init=initializer, name="q_heads")(x)
        if self.n_h_heads == 0:
            if self.n_heads > 1:
                return q_vals.reshape(self.n_heads, self.n_actions)
            else:
                return q_vals
        else:
            h_vals = nn.Dense(self.n_actions * self.n_h_heads, kernel_init=initializer, name="h_heads")(x)
