...
sbatch --job-name $EXPERIMENT_NAME-$ALGO_NAME --array=$FIRST_SEED-$LAST_SEED:$N_PARALLEL_SEEDS --cpus-per-task=1 --mem-per-cpu=$((N_PARALLEL_SEEDS * 1500))M --time=5:20:00 --partition stud \
