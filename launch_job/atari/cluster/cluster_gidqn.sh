...
sbatch --job-name $EXPERIMENT_NAME-$ALGO_NAME --array=$FIRST_SEED-$LAST_SEED:$N_PARALLEL_SEEDS --cpus-per-task=4 --mem-per-cpu=$((N_PARALLEL_SEEDS * 4500))M--time=48:00:00 --gres=gpu:1 --prefer="rtx3090|a5000" --exclude="dgx-station" --partition gpu \
