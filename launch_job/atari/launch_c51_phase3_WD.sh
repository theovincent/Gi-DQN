...
do
    ...
    do
          launch_job/atari/${PLATFORM}_c51rcshared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS \
              --learning_rate $LEARNING_RATE_C51RC \
              --weight_decay $WEIGHT_DECAY --experiment_name "LR${LEARNING_RATE_C51RC}_WD${WEIGHT_DECAY}_${GAME}"

          launch_job/atari/${PLATFORM}_gic51shared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS  \
              --learning_rate $LEARNING_RATE_GIC51  \
              --target_update_period $TARGET_UPDATE_PERIOD_GIC51 \
