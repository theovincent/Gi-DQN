SHARED_ARGS="--features 16 256 --replay_buffer_capacity 50_000 --batch_size 16 --update_horizon 1 \
    ...
    --n_initial_samples 10_000 --pixels 16 --n_frame_stack 2 \
    --n_frame_skip 4"
...
do
  for EPS_DURATION in 100_000 1_000_000
  do
    for EPS_END in 0.01 0.1
    do
    launch_job/atari/${PLATFORM}_dqnrcshared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS \
        --learning_rate $LEARNING_RATE_QRC  --epsilon_td --epsilon_duration $EPS_DURATION --epsilon_end $EPS_END \
        --weight_decay $WEIGHT_DECAY --experiment_name "LR${LEARNING_RATE_QRC}_WD${WEIGHT_DECAY}_EPSTD_DUR${EPS_DURATION}_END${EPS_END}_${GAME}"
    ...
    launch_job/atari/${PLATFORM}_gidqnshared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS \
        ...
        --n_bellman_iterations $N_BELLMAN_ITERATIONS --weight_decay $WEIGHT_DECAY --epsilon_td --epsilon_duration $EPS_DURATION --epsilon_end $EPS_END \
        --experiment_name "LR${LEARNING_RATE_GIDQN}_T${TARGET_UPDATE_PERIOD_GIDQN}_K${N_BELLMAN_ITERATIONS}_WD${WEIGHT_DECAY}_EPSTD_DUR${EPS_DURATION}_END${EPS_END}_${GAME}"
    done
  done
done