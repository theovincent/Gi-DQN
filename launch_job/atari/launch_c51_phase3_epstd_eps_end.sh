...
do
    ...
    do
    launch_job/atari/${PLATFORM}_epstdc51rcshared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS \
        --learning_rate $LEARNING_RATE_C51RC  --epsilon_end $EPS_END \
        --weight_decay $WEIGHT_DECAY --experiment_name "LR${LEARNING_RATE_C51RC}_WD${WEIGHT_DECAY}_END${EPS_END}_${GAME}"

    launch_job/atari/${PLATFORM}_epstdgic51shared.sh --first_seed 1 --last_seed 5 --n_parallel_seeds 1 $SHARED_ARGS \
        ...
        --experiment_name "LR${LEARNING_RATE_GIC51}_T${TARGET_UPDATE_PERIOD_GIC51}_K${N_BELLMAN_ITERATIONS}_WD${WEIGHT_DECAY}_END${EPS_END}_${GAME}"
    done
done
