...
class iSDQNShared:
    def __init__(
        ...
        self.networks = DQNNet(features, n_actions, n_heads=1 + self.n_bellman_iterations, n_h_heads=0)

        # initialize 1 network with K+1 heads
    ...
    def loss(self, params: FrozenDict, sample: ReplayElement, importance_weight):
        q_values = self.networks.apply(params, sample.state)[1:, sample.action]
        ...
        return importance_weight * jnp.square(td_errors), jnp.square(td_errors)
    ...
    def best_action(self, params: FrozenDict, state: jnp.ndarray):
        return jnp.argmax(self.networks.apply(params, state)[1:].mean(axis=0))
