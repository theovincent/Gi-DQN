...
class EPSTDC51RCShared:
    ...
    def best_action(self, params: FrozenDict, state: jnp.ndarray):
        q_logits, h_logits = self.network.apply(params, state)
        ...
        h_probabilities = jax.nn.softmax(h_logits, axis=-1)

        q_values = q_probabilities @ self.support  # expectation
        h_values = h_probabilities @ self.support

        q_action = jnp.argmax(q_values, axis=-1)
        h_action = jnp.argmax( jnp.abs(h_values), axis=-1)
