...
class ISF_iDQNShared:
    ...
    def loss(self, params: FrozenDict, target_params: FrozenDict, sample: ReplayElement, importance_weight):
        ...
        if not self.iterated_shared_features:
            ...
            q_values = self.online_networks.apply(params, sample.state)[:, sample.action][1:]
            next_q_values = self.online_networks.apply(params, sample.next_state)[:-1]
            targets = self.compute_target(next_q_values, sample)
    ...
    def best_action(self, params: FrozenDict, state: jnp.ndarray):
        ...
        q_outputs = self.online_networks.apply(params, state)
        if not self.iterated_shared_features:
